...
export default function WorkerDashboard() {
  ...
  const myTasks = tasks.filter(t => (t.assignedTo === currentUser.id || t.teamId === currentUser.teamId) && t.status !== 'completed')
        ...
        <DialogContent className="sm:max-w-md bg-zinc-950/90 border-white/10 text-white rounded-[3rem] backdrop-blur-3xl shadow-2xl">
          <DialogHeader className="pt-4">
            ...
            <DialogDescription className="text-white/60 text-md">
              Mark attendance for your team ({today}). This log is verified by your Zone Admin.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-6 max-h-[60vh] overflow-y-auto pr-2">
            {attendanceState.length === 0 ? (
              <p className="text-center py-10 text-white/40 italic">No members found in team roster. Please contact Zone Admin.</p>
            ) : (
              attendanceState.map((record, idx) => (
                <div key={record.workerId} className="p-5 rounded-3xl bg-white/5 border border-white/10 flex flex-col gap-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-[10px] text-white/40 font-bold uppercase tracking-widest">Team Member</span>
                  </div>
                  <RadioGroup 
          ...
          <div className="flex items-center gap-8 p-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-4">
                <p className="text-white/40 text-sm font-bold uppercase tracking-widest">Unit ID: {currentUser.id} • {currentUser.address || 'Central Area'}</p>
                {hasMarkedAttendance && <Badge className="bg-emerald-500/10 text-emerald-500 border-emerald-500/20 text-[10px] font-bold uppercase">Ready for Duty</Badge>}
              </div>
            </div>
          </div>

          <div className="space-y-6">
            <h3 className="font-headline font-bold text-2xl flex items-center gap-3 uppercase tracking-tighter">
                ...
                <Card key={task.id} className="rounded-[3rem] border-none shadow-2xl overflow-hidden glass-panel">
                  <CardHeader className="p-8 pb-4">
                    <div className="flex justify-between items-start">
                      <div>
                        <CardTitle className="text-2xl font-bold text-white mb-2 uppercase tracking-tighter">{task.work}</CardTitle>
                  ...
                  <CardFooter className="p-8 pt-0 gap-4">
                    {task.status === 'pending' ? (
                      <>
                        <Button className="flex-1 h-14 rounded-2xl bg-primary font-bold shadow-2xl shadow-primary/20 text-black" onClick={() => updateTask(task.id, { status: 'in_progress' })}>
                          <CheckCircle2 className="h-5 w-5 mr-2" /> Accept Work
                        </Button>
                        <Button variant="outline" className="flex-1 h-14 rounded-2xl border-rose-500/20 text-rose-500 bg-rose-500/5 font-bold" onClick={() => updateTask(task.id, { assignedTo: undefined, status: 'pending' })}>
                          <XCircle className="h-5 w-5 mr-2" /> Report Delay
                        </Button>
                      </>
                    ) : (
                      <Button className="w-full h-14 rounded-2xl bg-primary font-bold shadow-2xl shadow-primary/20 text-black" onClick={() => updateTask(task.id, { status: 'completed' })}>
                        <QrCode className="h-5 w-5 mr-2" /> Scan Verification QR
                      </Button>
                    )}
                  </CardFooter>
                </Card>
        ...
        <div className="space-y-10">
          <Card className="rounded-[3rem] border-none shadow-2xl bg-primary text-black">
             ...
             <CardContent className="space-y-6 pt-4">
                <div className="bg-black/10 p-8 rounded-[2rem] text-center border border-black/5">
                   <p className="text-[10px] font-bold uppercase tracking-widest mb-1">Total Points</p>
          ...
          <Card className="rounded-[3rem] border-none shadow-2xl glass-panel">
             ...
             <CardContent className="p-8 pt-0 space-y-4">
                {[
                  { label: 'High-Vis Vest', val: true },
                  { label: 'Safety Gloves', val: true },
                  { label: 'Dust Mask', val: true }
                ].map((item, i) => (
